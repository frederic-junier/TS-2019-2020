...
\title{Exercices complémentaires sur les nombres complexes}\subtitle{Terminale S}
...
\newcounter{exo}
\setcounter{exo}{0}
\newcommand{\exercice}[1]{\addtocounter{exo}{1}\frametitle{Exercice \theexo  \textit{ thème : #1}}}
...
A tout  nombre complexe non nul $z=x + \ic y$ avec $x$ et $y$ réels, on associe le nombre complexe $f(z)=\frac{z-\ic}{z}$.
\begin{enumerate}
\item Démontrer que pour tout nombre complexe $z \neq 0$, on a $f(z)=\frac{x^2+y^2-y}{x^2+y^2} - \ic \frac{x}{x^2+y^2}$.
\item Dans le plan muni d'un repère orthonormal \Ouv, déterminer l'ensemble $\Gamma$ des points $M$ de coordonnées $\Coord{x}{y}$ tels  que $f(z)$ soit un réel pur.
\item Dans le plan muni d'un repère orthonormal \Ouv, déterminer l'ensemble $\Gamma$ des points $M$ de coordonnées $\Coord{x}{y}$ tels  que $f(z)$ soit un imaginaire pur.
